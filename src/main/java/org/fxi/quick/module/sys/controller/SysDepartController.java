...
public class SysDepartController {
	...
	public Result<List<SysDepartTreeModel>> queryMyDeptTreeList() {
		...
		try {
			if(user.getUserIdentity() != null && user.getUserIdentity().equals( CommonConstant.USER_IDENTITY_2 )){
...
//			}
    ...
    */
	...
	@CacheEvict(value= {CacheConstant.SYS_DEPARTS_CACHE,CacheConstant.SYS_DEPART_IDS_CACHE}, allEntries=true)
   public Result<SysDepart> delete(@RequestParam(name="id",required=true) Long id) {
	...
	public Result<SysDepart> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
		...
		if (ids == null || "".equals(ids.trim())) {
			...
			this.sysDepartService.deleteBatchWithChildren(Arrays.asList(ids.split(",")).stream()
					.map(id -> Long.valueOf(id)).collect(
					Collectors.toList()));
			result.success("删除成功!");
		}
		return result;
